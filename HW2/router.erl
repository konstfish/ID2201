...
add(Name, Ref, Pid, Intf) ->
   sets:add_element({Name, Ref, Pid}, Intf).

% remove an entry given a name of an instance, return a new set of interfaces
remove(Name, Intf) ->
    case {lookup(Name, Intf), ref(Name, Intf)} of
        {{ok, Pid}, {ok, Ref}} ->
            sets:del_element({Name, Ref, Pid}, Intf);
        _ -> % unchanged
            Intf
    end.
...
lookup(Name, Intf) ->
  case sets:to_list(sets:filter(fun({N, _Ref, _Pid}) -> N =:= Name end, Intf)) of
    [{_Name, _Ref, Pid}] ->
      {ok, Pid};
    [] ->
      notfound
...
name(Ref, Intf) ->
  case sets:to_list(sets:filter(fun({_Name, R, _Pid}) -> R =:= Ref end, Intf)) of
    [{Name, _Ref, _Pid}] ->
      {ok, Name};
    [] ->
      notfound
...
list(Intf) ->
  lists:foldl(fun({Name, _Ref, _Pid}, Acc) -> Acc ++ [Name] end, [], sets:to_list(Intf)).
